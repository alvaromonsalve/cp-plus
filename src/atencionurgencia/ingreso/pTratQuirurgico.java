 ...
 */
public class pTratQuirurgico extends javax.swing.JPanel {
    ...
    /**
     * Creates new form pTratQuirurgico
     */
    public pTratQuirurgico() {
        initComponents();
        setCargaTabla();
    }
    ...
    public void saveChanges(EntityManagerFactory factory, InfoHistoriac ihc){
        ...
        for(int i=0;i<ModeloTabla.getRowCount();i++){
            ...
            boolean exist=false;
            for(int a=0;a<listInfoProcedimientoHcu.size();a++){
                if(((ConfigCups)ModeloTabla.getValueAt(i, 0)).getId()==listInfoProcedimientoHcu.get(a).getIdCups()){
                    exist=true;
                    procedimientoHcu=listInfoProcedimientoHcu.get(a);
                    break;
                }
            }
            if(!exist){
                ...
                if(procedimientoHcu!=null){
                    ...
                    try {
                        ...
                        JOptionPane.showMessageDialog(null, "10088:\n"+ex.getMessage(), pTratQuirurgico.class.getName(), JOptionPane.INFORMATION_MESSAGE);
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(null, "10089:\n"+ex.getMessage(), pTratQuirurgico.class.getName(), JOptionPane.INFORMATION_MESSAGE);
                    }
                }
            }
        }
        for(int i=0;i<listInfoProcedimientoHcu.size();i++){
            ...
            if(cups.getIdEstructuraCups().getId() >0 &&cups.getIdEstructuraCups().getId() <=14){
                boolean exist=false;
                for(int a=0;a<ModeloTabla.getRowCount();a++){
                    if(listInfoProcedimientoHcu.get(i).getIdCups()==((ConfigCups)ModeloTabla.getValueAt(a, 0)).getId()){
                        exist=true;
                        break;
                    }
                }
                if(!exist){
                    try {
                        infoProcedimientoHcuJPA.destroy(listInfoProcedimientoHcu.get(i).getId());
                    } catch (NonexistentEntityException ex) {
                        JOptionPane.showMessageDialog(null, "10090:\n"+ex.getMessage(), pTratQuirurgico.class.getName(), JOptionPane.INFORMATION_MESSAGE);
                    }
                }
            }
        }
    ...
    private void initComponents() {
        ...
        jLabel50.setText("Procedimientos e intervenciones quirúrgicos");
    ...
    private void jTable1MouseMoved(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTable1MouseMoved
        ...
        jTable1.setToolTipText("<html>\n" +
            "<div style=\"width:400;\">"+(String)jTable1.getValueAt(rowIndex, 2)+"</div>\n" +
            "\n" +
            "</html>");
